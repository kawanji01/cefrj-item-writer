...
語彙形式の対象出現照合フィールドに限り、MC-19の宣言対象候補を第1段の一般複数語候補と同時に比較する。これは新しい先行段階を追加するものではなく、MC-14の最長一致を維持した上で同長のタイブレークだけを対象ID優先にする。対象候補が1トークンで、より長い一般複数語候補がない場合は、その対象候補を消費してMC-15へ渡さない。非対象フィールドでは従来の順序だけを適用する。

`vocab_mcq_ja2en`の`body.choices[*].text`に限り、MC-23第1項を満たす実在anchorのheadwordを宣言アンカー候補とする。headwordを固定spaCyモデルでトークン列化し、選択肢フィールド全体がその表層形列と一致した場合は、複数トークンへ分割されても区間全体を同じanchorエントリで消費する。MC-23第1項を満たさないanchorにはこの候補を作らず、通常の照合順序とMC-23違反を適用する。他形式・他フィールドには適用しない。
...
2. 同一開始位置で複数エントリが一致する場合はトークン数が最大のものを採用する。トークン数も同じ場合は、対象出現照合フィールドでMC-19の宣言対象候補が含まれればその対象IDを採用し、それ以外は `id` の辞書順で先のエントリを採用する。
...
対象出現照合フィールドでは、MC-19の宣言対象候補により消費済みの単一トークンを本節で再照合しない。これにより、表層形がheadwordと一致する原本見出しはレンマ・品詞候補より宣言対象を優先し、活用形は補正後レンマ列で同じ対象候補に一致させる。
...
1. `anchor.entry_id` がlexiconに存在し、記録された `headword` / `pos` / `level` がエントリの実値と一致する。`vocab_mcq_ja2en` では選択肢の英語表記 `text` が由来エントリの `headword` と一致する（小文字化比較）。成立時はMC-13の宣言アンカー候補として、選択肢フィールド全体の各トークンを同じanchorエントリ・`decision = "wordlist_match"`で記録する。不成立時は宣言アンカー候補を作らない。不成立は `V-DIS-01`。
2. 誤答3肢の `anchor.level` = 指定レベル `L` である（LVL-13）。誤答の `anchor.pos` がターゲットと同一である。ただし `body.pos_pool_relaxed = true` の場合、品詞同一性検査に代えて `docs/question-generation-spec.md` GEN-15 の互換品詞群（同一群内）であること、および少なくとも1つの誤答アンカーがターゲットと異なるposであることを検査する。機械検査は同品詞の生候補数または意味的有効件数から緩和の必要性を推定しない。GEN-13の意味的除外後に有効候補が3語未満であることはLLMレビューのCHK-06が検証する。`pos_pool_relaxed = false` なのに品詞が不一致の場合も本項の違反である。不成立は `V-DIS-02`。
