...
- **NFR-04 オフライン動作とテレメトリ**: セットアップ完了後の決定的スクリプトとテスト本体は完全オフラインで動作しなければならない(MUST)。例外処理は、製品セットアップでの固定版依存パッケージとspaCyモデルの取得、およびCI・開発環境でテスト開始前に行う`requirements-dev.txt`の固定pytest取得に限りネットワーク接続してもよい(MAY)。テレメトリ送信をしてはならない(MUST NOT)（D-22、M7D-07、M8D-02）。LLM呼び出し（生成・レビュー）はホストツール経由のネットワーク通信を伴い、この要件の対象外である。
