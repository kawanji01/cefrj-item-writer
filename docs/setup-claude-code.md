...
Claude Codeが起動したら、プロジェクトを信頼し、`/agents` を開きます。成功時はLibraryにプロジェクトサブエージェント `cefrj-reviewer` が表示されます。これはレビュアー配線の宣言確認用であり、標準フローの独立レビューは`.claude/run_reviewer.py`が`safe-mode`の新規`claude -p`を起動し、生stdout/stderrをシェル不使用で`flow_control.py review`へ直接渡します。続いて `/permissions` を開き、`.claude/settings.json` が読み込まれ、リポジトリ読取り、`output/` 書込み、9本のPython CLI実行、ホスト側`flow_control.py`の固定init/candidate/decide/status呼出し、`output/<set_id>/.staging/`の固定candidate rawとCLI専用flow-state、固定requestパスだけを受ける期限付きレビュアー起動、およびS80開始時の識別子生成だけを行う引数なし固定コマンドが標準フロー用に許可され、Web取得・検索が拒否されていることを確認してください。`REV02`/`ERR02`ヒアドキュメントによる直接review呼出しは許可しません。Bashサンドボックスの通信許可は、固定レビュアー子プロセスがモデルを呼び出す`api.anthropic.com`だけです。別ドメインやワイルドカードを追加せず、`WebFetch` / `WebSearch`の拒否を維持してください。candidate rawは監査正本またはinvalid監査の保存後に`flow_control.py`が削除し、flow-stateは同CLIだけが管理します。汎用`rm`、flow-stateの手動編集、パイプ・追加コマンドを許可しません。識別子生成コマンドの許可は全文完全一致で、汎用`python -c`や追加引数を許可しません。`.claude/skills/cefrj-author/SKILL.md`、`.claude/agents/cefrj-reviewer.md`、`.claude/run_reviewer.py` はリポジトリに同梱されています。
